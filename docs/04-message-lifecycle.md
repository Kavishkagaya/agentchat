...
3. **Briefing:** The Chat Controller sends the **Summary + Recent Window** to the **Agents Worker (shared)**.
4. **Action:** The Agents Worker decides if it needs a Sandbox (e.g., "I need to test this React component"). Agent configs and secrets are fetched on demand and cached in memory for low latency.
