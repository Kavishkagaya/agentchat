...
The system SHALL require MCP URL, token, and required metadata when adding an MCP server.
...
When an MCP server is added, the system MUST validate it by fetching its tool list in a worker-only environment.

#### Scenario: Validate MCP server
- **WHEN** an MCP server is added
- **THEN** the system fetches tools and records validation status
