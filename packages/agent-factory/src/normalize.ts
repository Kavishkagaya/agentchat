...
export function normalizeAgentConfig(agentId: string, raw: Record<string, unknown>): AgentConfig {
  const systemPrompt =
    (raw.systemPrompt as string | undefined) ?? (raw.system_prompt as string | undefined);
  ...
  return {
    ...
    max_tokens: (raw.maxTokens as number | undefined) ?? (raw.max_tokens as number | undefined),
    top_p: (raw.topP as number | undefined) ?? (raw.top_p as number | undefined),
    presence_penalty:
